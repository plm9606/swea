...
/*
   사용하는 클래스명이 Solution 이어야 하므로, 가급적 Solution_1213.java 를 사용할 것을 권장합니다.
...
//                pan.add(sc.next());
            ...
            for(int r=0; r<8; r++){
                ArrayList<String> row = pan.get(r);
                for(int i=length-1; i<8; i++){
                    ...
                    if(length%2==0) {
                        for (int j = 0; j < length / 2; j++) {
                            if (!row.get(i - j).equals(row.get(i - (length - j - 1)))) {
                                MATCH = false;
                                break;
                            }
                        }
                        ...
                        for(int j=0; j< length/2; j++){
                            if(!pan.get(i-j).get(r).equals(pan.get(i-(length-1-j)).get(r))){
                                COLMATCH = false;
                                break;
                            }
                        }
                        ...
                        for(int j=0; j<((length-1)/2); j++){
                            if(!row.get(i-j).equals(row.get(i-length+1+j))){
                                MATCH = false;
                                break;
                            }
                        }
